...
namespace BookCurlPro.BookEditor
{
    ...
    public class BookProEditor : Editor
    {
        ...
        private void DrawElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            // 밝기(다크/라이트) 테마에 맞춘 배경 및 레이블 색상 사용
            Color bgColor = EditorGUIUtility.isProSkin ? new Color(0.15f, 0.15f, 0.15f) : new Color(0.9f, 0.9f, 0.9f);
            EditorGUI.DrawRect(new Rect(rect.x, rect.y, rect.width, rect.height - 6), bgColor);
            GUIStyle labelStyle = new GUIStyle(EditorStyles.label);
            labelStyle.normal.textColor = EditorGUIUtility.isProSkin ? Color.white : Color.black;
            EditorGUI.LabelField(new Rect(rect.x, rect.y, 100, EditorGUIUtility.singleLineHeight), "Paper#" + index, labelStyle);

            if (index == book.CurrentPaper)
                EditorGUI.DrawRect(new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, 140, EditorGUIUtility.singleLineHeight), EditorGUIUtility.isProSkin ? new Color(0.55f, 0.15f, 0.15f) : new Color(1f, 0.3f, 0.3f));
            EditorGUI.LabelField(new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, 40, EditorGUIUtility.singleLineHeight), "Front:", labelStyle);
            ...
            if (index == book.CurrentPaper - 1)
                EditorGUI.DrawRect(new Rect(rect.x, rect.y + 3 * EditorGUIUtility.singleLineHeight, 140, EditorGUIUtility.singleLineHeight), EditorGUIUtility.isProSkin ? new Color(0.55f, 0.15f, 0.15f) : new Color(1f, 0.3f, 0.3f));
            EditorGUI.LabelField(new Rect(rect.x, rect.y + 3 * EditorGUIUtility.singleLineHeight, 35, EditorGUIUtility.singleLineHeight), "Back:", labelStyle);
            EditorGUI.PropertyField(
