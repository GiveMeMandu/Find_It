...
#endif
                float shape = 1.0 - step(length(offset), _Pixels.z * _Roundness) * step(max(offset.x, offset.y), _Pixels.z);
                _Color.a *= sample.a;
				return lerp(sample, _Color, shape);
				// return lerp(sample, lerp(sample, _Color, _Color.a), shape); // option with color blending
            }
            ENDHLSL
        }
    }
