...
{
    private List<HiddenObjUI> allHiddenObjUIs => levelManager.GetAllHiddenObjUIs();
    ...
    {
        foreach (var hiddenObjUI in allHiddenObjUIs)
        {
            if (hiddenObjUI != null)
            {
        ...
        Debug.Log($"[ShadowModeManager] {allHiddenObjUIs.Count}개의 UI가 실루엣 모드로 전환되었습니다");
    }
