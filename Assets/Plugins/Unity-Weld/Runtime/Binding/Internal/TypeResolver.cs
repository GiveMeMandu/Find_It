...
namespace UnityWeld.Binding.Internal
{
    ...
    public static class TypeResolver
    {
        ...
        public static string GetViewModelBindingName(AbstractMemberBinding binding)
        {
            if (binding is OneWayPropertyBinding)
            {
                return (binding as OneWayPropertyBinding).ViewModelPropertyName;
            }
            else if (binding is TwoWayPropertyBinding)
            {
                return (binding as TwoWayPropertyBinding).ViewModelPropertyName;
            }
            else if (binding is ToggleActiveBinding)
            {
                return (binding as ToggleActiveBinding).ViewModelPropertyName;
            }
            else if (binding is EventBinding)
            {
                return (binding as EventBinding).ViewModelMethodName;
            }
            else if (binding is CollectionBinding)
            {
                return (binding as CollectionBinding).ViewModelPropertyName;
            }
            else if (binding is SubViewModelBinding)
            {
                return (binding as SubViewModelBinding).ViewModelPropertyName;
            }
            else if (binding is AnimatorParameterBinding)
            {
                return (binding as AnimatorParameterBinding).ViewModelPropertyName;
            }
            else
            {
                return string.Empty;
        ...
        public static bool IsWrongBinding(AbstractMemberBinding binding)
        {
            var bindingName = GetViewModelBindingName(binding);
            ...
            if (binding is EventBinding)
            {
                if (Array.Exists(FindBindableMethods(binding as EventBinding), x => x.ToString().Equals(bindingName)))
                {
                    return false;
                }
            }
            ...
            {
                if (Array.Exists(FindBindableProperties(binding), x => x.ToString().Equals(bindingName)))
                {
                    return false;
                }
            }
