...
namespace UnityWeld.Binding
{
    ...
    public abstract class AbstractMemberBinding : MonoBehaviour, IMemberBinding
    {
                                                       ...
                                                       out object viewModel)
        {
            ...
            if(viewModel == null)
            {
                throw new ViewModelNotFoundException("(Unity-Weld) Failed to find view-model in hierarchy: " + 
                gameObject.name + viewModelName);
            }
        }
        ...
        protected void ParseViewEndPointReference(string endPointReference, out string memberName, out Component view)
        {
            ...
            if(view == null)
            {
                throw new ComponentNotFoundException("(Unity-Weld) Failed to find component on '" 
                + gameObject.name + "' game object, Type : " + boundComponentType);
            }
        }
